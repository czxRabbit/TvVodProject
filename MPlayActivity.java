...
public class MPlayActivity extends Activity {
    ...
    protected void onResume ()
    {
        ...
        if(mplayUrl != null) {
            ...
            Toast.makeText(MPlayActivity.this,"上下键快进快退，确定键暂停，右键开始播放",Toast.LENGTH_LONG).show();
        }
        else{
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (keyCode == 23 || keyCode == 66){
            ...
            Toast.makeText(MPlayActivity.this,"当前位置 "+ijkPlayer.getCurrentPosition()/60000+"分钟",Toast.LENGTH_SHORT).show();
            ijkPlayer.pause();
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        if (keyCode == 19){ //up
            ...
            long percent = 100*ijkPlayer.getCurrentPosition()/ijkPlayer.getDuration();
            if (percent > 100){
                Toast.makeText(MPlayActivity.this,"播放完成 ",Toast.LENGTH_SHORT).show();
            }else{
                Toast.makeText(MPlayActivity.this,"播放进度 "+ijkPlayer.getCurrentPosition()/60000+"分钟"+" 总时长 "+ijkPlayer.getDuration()/60000+"分钟 "+percent+"%",Toast.LENGTH_SHORT).show();
            }
        }
        if (keyCode == 20) { //down
            Log.d(TAG, "down pressed");
